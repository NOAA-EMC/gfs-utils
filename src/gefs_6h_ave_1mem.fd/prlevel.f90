subroutine prlevel(ipdtn,ipdtmpl,labbrev)
      ...
      if ( ipdtmpl(ipos) .eq. 100 .and.ipdtmpl(ipos+3) .eq. 255 ) then           ! Pressure Level
         !write(tmpval1,*) ipdtmpl(ipos+2)/100
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1)+2)
         labbrev=trim(tmpval1)//" mb"

      elseif ( ipdtmpl(ipos) .eq. 100 .and.  ipdtmpl(ipos+3) .eq. 100 ) then         ! Pressure Layer
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1)+2)
      ...
      elseif ( ipdtmpl(ipos) .eq. 102 .and. ipdtmpl(ipos+3) .eq. 255 ) then       ! Altitude above MSL
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         labbrev=trim(tmpval1)//" m above MSL"

      elseif ( ipdtmpl(ipos) .eq. 103 .and. ipdtmpl(ipos+3) .eq. 255 ) then        ! Height above Ground
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         labbrev=trim(tmpval1)//" m above ground"

      elseif ( ipdtmpl(ipos) .eq. 103 .and.  ipdtmpl(ipos+3) .eq. 103 ) then        ! Height above Ground
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         call frmt(tmpval2,ipdtmpl(ipos+5),ipdtmpl(ipos+4))
         labbrev=trim(tmpval1)//" - "//trim(tmpval2)//" m AGL"

      elseif ( ipdtmpl(ipos) .eq. 104 .and. ipdtmpl(ipos+3) .eq. 255 ) then       ! Sigma Level
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         labbrev=trim(tmpval1)//" sigma"

      elseif ( ipdtmpl(ipos) .eq. 104 .and. ipdtmpl(ipos+3) .eq. 104 ) then        ! Sigma Layer
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         call frmt(tmpval2,ipdtmpl(ipos+5),ipdtmpl(ipos+4))
         labbrev=trim(tmpval1)//" - "//trim(tmpval2)//" sigma"

      elseif ( ipdtmpl(ipos) .eq. 105 .and. ipdtmpl(ipos+3) .eq. 255 ) then        ! Hybrid Level
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         labbrev=trim(tmpval1)//" hybrid lvl"

      elseif ( ipdtmpl(ipos).eq.105 .and. ipdtmpl(ipos+3).eq.105) then         ! Hybrid Level
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         call frmt(tmpval2,ipdtmpl(ipos+5),ipdtmpl(ipos+4))
         labbrev=trim(tmpval1)//" - "//trim(tmpval2)//" hybrid lvl"


      elseif ( ipdtmpl(ipos) .eq. 106 .and. ipdtmpl(ipos+3) .eq. 255 ) then      ! Depth Below Land Sfc
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         labbrev=trim(tmpval1)//" m below land"

      elseif ( ipdtmpl(ipos).eq.106 .and. ipdtmpl(ipos+3).eq.106) then         ! Depth Below Land Sfc Layer
         call frmt(tmpval1,ipdtmpl(ipos+2),ipdtmpl(ipos+1))
         call frmt(tmpval2,ipdtmpl(ipos+5),ipdtmpl(ipos+4))
      ...
      elseif ( ipdtmpl(ipos).eq.108 .and. ipdtmpl(ipos+3).eq.108) then      ! Press Diff from Ground Layer
         !write(tmpval1,*) ipdtmpl(ipos+2)/100
      ...
      elseif ( ipdtmpl(ipos).eq.109 .and. ipdtmpl(ipos+3).eq.255) then         ! Potential Vorticity Sfc
