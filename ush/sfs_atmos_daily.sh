...
do
  ...
  # shellcheck disable=SC2086
  ${GMERGE} - ${list} | wgrib2 - -match "${dailyaccvars}" -merge_fcst 4 "${OUTDIR}/acc.daily.${ENS}/acc.daily.${filename_start}${filemm}${filename_end}"
  ...
  do
    ...
    # shellcheck disable=SC2086
    ${GMERGE} - ${list_6hrly} | wgrib2 - -match "${dailyinstvars}" -fcst_ave 6hr "${OUTDIR}/inst.daily.${ENS}/daily_${end_hr}.grb"
  done
  ...
  # shellcheck disable=SC2086
  ${GMERGE} - ${list_daily} | wgrib2 - -grib "${OUTDIR}/inst.daily.${ENS}/inst.daily.${filename_start}${filemm}${filename_end}"
...
do
  ...
  #merge the min/max/acc/ave variables into daily periods
  # shellcheck disable=SC2086
  ${GMERGE} - ${list} | wgrib2 - -match "${dailyaccvars}" -merge_fcst 4 "${OUTDIR}/acc.daily.${ENS}/acc.daily.${filename_start_next}${filemm}${filename_end}"
  ...
  #### merge all days into single grib2 file and remove unneeded files
  # shellcheck disable=SC2086
  ${GMERGE} - ${list_daily} | wgrib2 - -grib "${OUTDIR}/inst.daily.${ENS}/inst.daily.${filename_start_next}${filemm}${filename_end}"
  rm "${OUTDIR}"/inst.daily."${ENS}"/daily*.grb
